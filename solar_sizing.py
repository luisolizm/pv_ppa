...
def _run_word_script(script_path: str, data: dict) -> bytes:
    ...
    import os as _os
    with _tempfile.NamedTemporaryFile(suffix=".json", delete=False, mode="w") as _jf:
        _json.dump(data, _jf, ensure_ascii=False, default=str)
        _jf_path = _jf.name
    _os.replace(_jf_path, "/tmp/word_data.json")
    _out = "/tmp/word_output.docx"
    ...
    if res.returncode != 0 or not _os.path.exists(_out):
        raise RuntimeError(f"Node error: {res.stderr[:400]}")
...
def build_word_turnkey(
    ...
    data = {
        ...
        "vpn":             vpn, "tir_str": f"{tir:.1f}%" if tir else "N/A",
...
with tab1:
    ...
    with col_p:
        ...
        uso_8760csv = False   # eliminado — usa el tab ☀️ Recurso Solar 8760h
...
""", unsafe_allow_html=True)
    ...
    for pl in ppa_plazos:
        ...
        res = dict(calc_ppa_result(precio_ppa=ppa_precio_manual, plazo=pl, **_kwargs_pl))
...
""", unsafe_allow_html=True)
    ...
    with sol_c1:
        ...
        sol_pr = st.slider(
            "Performance Ratio (PR)", 0.60, 0.95, float(effective_pr), 0.01,
            key="sol_pr",
            help="PR global del sistema (inversor, cableado, temperatura, suciedad). Hereda el valor del sidebar."
        )
...
""", unsafe_allow_html=True)
        ...
        if "sol_tmy_df" in st.session_state:
            ...
            # ── Simular generación PV hora a hora con corrección térmica ──────
            _irr_arr  = _tmy["irradiance_Wm2"].values[:8760]
            _temp_arr = _tmy["temp_C"].values[:8760]

            # Temperatura de celda: modelo NOCT (IEC 61724)
            # T_cell = T_amb + (NOCT - 20) / 800 * G
            _t_cell = _temp_arr + ((_noct - 20.0) / 800.0) * _irr_arr
            # Factor de corrección por temperatura
            _temp_factor = 1.0 + (_gamma / 100.0) * (_t_cell - 25.0)
            _temp_factor = _temp_factor.clip(min=0.0)
            # Generación por kWp: G[W/m²]/1000 * PR * factor_temp
            _pv_per_kwp = (_irr_arr / 1000.0) * _pr * _temp_factor   # kW/kWp hora a hora
            _pv_kw      = _pv_per_kwp * _kwp                          # kW total

            # ── Estadísticas globales ─────────────────────────────────────────
            _irr_total_kwh = float((_irr_arr / 1000.0).sum())   # kWh/m²/año (≈ kWh/m²)
            _gen_total_kwh = float(_pv_kw.sum())                 # kWh/año
            _gen_per_kwp   = float(_pv_per_kwp.sum())            # kWh/kWp/año
            _pr_efectivo   = _gen_per_kwp / max(_irr_total_kwh, 1) * 100  # % PR real
            _temp_media    = float(_temp_arr.mean())
            _peak_pv       = float(_pv_kw.max())
            _horas_gen     = int((_pv_kw > 0.01).sum())
            _factor_cap    = _gen_total_kwh / (_kwp * 8760) * 100   # % factor de planta

            st.markdown(f"""
<div class="tor-hero" style="margin-top:0.5rem;">
  <div class="th-project">🌞 RECURSO SOLAR — TMY 8760h · ({lat:.4f}, {lon:.4f})</div>
  <div class="th-meta">NASA POWER climatología {NASA_START}–{NASA_END} · Año ref {sol_year} ·
    PR {_pr*100:.0f}% · NOCT {_noct}°C · γ {_gamma:.2f}%/°C · {_kwp:.0f} kWp
  </div>
  <div class="th-grid" style="grid-template-columns:repeat(4,1fr);">
    <div class="th-item">
      <span class="th-label">IRRADIACIÓN ANUAL</span>
      <span class="th-val">{_irr_total_kwh:,.0f}</span>
      <span class="th-unit">kWh/m²/año · GHI horizontal</span>
    </div>
    <div class="th-item">
      <span class="th-label">GENERACIÓN ANUAL</span>
      <span class="th-val">{_gen_total_kwh/1000:,.1f}</span>
      <span class="th-unit">MWh/año · {_kwp:.0f} kWp instalados</span>
    </div>
    <div class="th-item">
      <span class="th-label">GEN. ESPECÍFICA</span>
      <span class="th-val">{_gen_per_kwp:,.0f}</span>
      <span class="th-unit">kWh/kWp/año</span>
    </div>
    <div class="th-item">
      <span class="th-label">PR EFECTIVO</span>
      <span class="th-val">{_pr_efectivo:.1f}</span>
      <span class="th-unit">% real (con corrección térmica)</span>
    </div>
    <div class="th-item" style="margin-top:10px;">
      <span class="th-label">TEMP. MEDIA ANUAL</span>
      <span class="th-val">{_temp_media:.1f}</span>
      <span class="th-unit">°C · NASA POWER T2M</span>
    </div>
    <div class="th-item" style="margin-top:10px;">
      <span class="th-label">HORAS DE GENERACIÓN</span>
      <span class="th-val">{_horas_gen:,}</span>
      <span class="th-unit">h/año con PV > 10 W</span>
    </div>
    <div class="th-item" style="margin-top:10px;">
      <span class="th-label">PICO DE GENERACIÓN</span>
      <span class="th-val">{_peak_pv:,.1f}</span>
      <span class="th-unit">kW · hora más soleada del año</span>
    </div>
    <div class="th-item" style="margin-top:10px;">
      <span class="th-label">FACTOR DE PLANTA</span>
      <span class="th-val">{_factor_cap:.1f}</span>
...
""", unsafe_allow_html=True)
            ...
            # ── Heatmap GHI 24 × 365 ─────────────────────────────────────────
            st.markdown('<div class="section-header">Heatmap de irradiancia GHI (W/m²)</div>',
                        unsafe_allow_html=True)
            st.plotly_chart(
            ...
            # ── Heatmap Generación PV ─────────────────────────────────────────
            st.markdown('<div class="section-header">Heatmap de generación PV (kW)</div>',
                        unsafe_allow_html=True)
            st.plotly_chart(
            ...
            st.markdown('<div class="section-header">Perfil mensual — irradiación y generación</div>',
                        unsafe_allow_html=True)

            # Agregar por mes: sumar kWh/m² y MWh generados
            _months_irr = []
            _months_gen = []
            _day_count  = [0]*12
            _ptr = 0
            for m_idx, days in enumerate(MONTH_DAYS):
                ...
                _months_gen.append(float(_pv_kw[_ptr:_ptr+hrs].sum()) / 1000.0)  # MWh
                _ptr += hrs
            ...
            _fig_mes.add_trace(go.Scatter(
                ...
                name=f"Generación PV (MWh) · {_kwp:.0f} kWp",
                mode="lines+markers",
            ...
            _lay_mes.update({
                ...
                "yaxis2": dict(title="Generación PV (MWh)", overlaying="y", side="right",
            ...
            st.markdown('<div class="section-header">Perfil horario diario promedio (kW/kWp)</div>',
                        unsafe_allow_html=True)
            # Calcular promedio por hora del día para cada mes
            _pv_per_kwp_arr = _pv_per_kwp  # kW/kWp
            _fig_horal = go.Figure()
            _horas_dia = list(range(24))
            _ptr = 0
            _colors_month = [
            ...
            # ── P50 / P90 interanual (si hay datos NASA mensuales) ────────────
            if active_irr_por_anio:
                ...
                if p50_r is not None:
                    ...
                    _p50_mwh = p50_r / 1000
                    _p90_mwh = p90_r / 1000
                    _bar_col = [ROSE if v < _p90_mwh else AMBER if v < _p50_mwh else TEAL for v in _gen_v]
                    _fig_p90 = go.Figure()
                    _fig_p90.add_trace(go.Bar(
                        x=_anios_v, y=_gen_v, marker=dict(color=_bar_col, cornerradius=4),
                    ...
                    _lay_p9.update({
                        ...
                        "xaxis": dict(tickmode="linear", dtick=1, tickangle=-45, gridcolor="rgba(0,0,0,0)"),
                        "bargap": 0.25,
                        "margin": dict(l=20, r=20, t=20, b=60),
                    })
                ...
                st.markdown(
                    '<div class="info-box">Carga datos NASA POWER desde el sidebar para calcular '
                    'la variabilidad interanual P50/P90 con los 20 años históricos.</div>',
                    unsafe_allow_html=True
                )

            # ── Exportar CSV 8760h ────────────────────────────────────────────
            st.markdown('<div class="section-header">Exportar CSV 8760h</div>',
                        unsafe_allow_html=True)
            ...
            st.markdown("""
<div class="info-box" style="font-size:12px;">
  📊 <b>Columnas del CSV:</b>
  <code>hora_del_año</code> (1–8760) ·
  <code>datetime_TMY</code> · <code>GHI_Wm2</code> ·
  <code>temp_amb_C</code> · <code>temp_celula_C</code> · <code>factor_temp</code> ·
  <code>gen_kW_por_kWp</code> (escala a cualquier tamaño) ·
  <code>gen_kW_{kWp}kWp</code> (para los kWp configurados).
  Escala: multiplica <code>gen_kW_por_kWp</code> × tu kWp.
</div>
""".replace("{kWp}", f"{_kwp:.0f}"), unsafe_allow_html=True)

            st.download_button(
                f"⬇️ Descargar CSV 8760h — {_kwp:.0f} kWp · ({lat:.4f}, {lon:.4f})",
                data=_csv_bytes,
            ...
            st.markdown("""
<div class="info-box" style="font-size:11px;margin-top:6px;">
  <b>Metodología:</b> TMY construido con irradiancia horaria NASA POWER (API temporal/hourly)
  para el año de referencia, escalada mes a mes al promedio climatológico {start}–{end}.
  Temperatura de celda: modelo NOCT IEC 61724 (T_cell = T_amb + (NOCT−20)/800 × G).
  Corrección de potencia: factor = 1 + γ × (T_cell − 25°C).
  El PR declarado incluye pérdidas de inversor, cableado y mismatch — la corrección
  térmica se aplica adicionalmente, por lo que el PR efectivo puede diferir del declarado.
</div>
""".format(start=NASA_START, end=NASA_END), unsafe_allow_html=True)
