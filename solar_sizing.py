...
""", unsafe_allow_html=True)
        ...
        _has_recibo = (not uso_area) and monthly_cons_ref is not None and sum(monthly_cons_ref or []) > 0
        if _has_recibo:
            ppa_fuente = st.radio(
                "Fuente de Capacidad, Generación e Inversión",
                ["📐 Turnkey Solar", "🧾 Recibo CFE"],
                horizontal=True,
                key="ppa_fuente",
                help="Turnkey Solar: usa kWp, generación e inversión del sizing por área/turnkey. Recibo CFE: usa los datos del historial de consumo del recibo.",
            )
            _usar_recibo = (ppa_fuente == "🧾 Recibo CFE")
        else:
            _usar_recibo = False
            st.caption("ℹ️ Para habilitar la fuente 'Recibo CFE', usa el modo 'Por datos del recibo CFE' en la pestaña Turnkey Solar.")

        # Valores según fuente seleccionada
        if _usar_recibo:
            _kwp_fuente       = max(1.0, round(float(kwp), 1))
            _p50_val          = max(100.0, round(float(annual_gen), 0))
            _p90_val          = max(100.0, round(float(p90_real), 0)) if p90_real else None
            _inv_fuente       = max(1000.0, round(_kwp_fuente * float(costo_kwp), 0))
            _fuente_label     = "Recibo CFE"
            _fuente_color     = "nasa-box"
            _fuente_icon      = "🧾"
        else:
...
""", unsafe_allow_html=True)
        ...
        ppa_inversion_usd = st.number_input("Inversión total (USD)", 1000.0, 50_000_000.0,
                                             float(_inv_fuente), 100.0, key="ppa_inv")
    ...
    for pl in ppa_plazos:
        ...
        # LCOE PPA: (CAPEX_total + PV O&M+seguros) / PV generación — descontado a WACC (estándar IEA/NREL)
        # Se usa WACC, no Ke, porque el LCOE es métrica del proyecto completo (deuda+equity),
        # no solo de la perspectiva equity. Consistente con calc_financial_model (Turnkey).
        _wacc_r  = ppa_wacc / 100
        _pv_gen  = sum(res_base["gen_y"][i] / (1 + _wacc_r) ** (i + 1) for i in range(pl))
        _pv_cost = res_base["inv_mxn"] + sum(
            (res_base["om_y"][i] + res_base["seg_y"][i]) / (1 + _wacc_r) ** (i + 1)
            for i in range(pl)
        )
        res["lcoe"] = _pv_cost / _pv_gen if _pv_gen > 0 else 0.0
        # PI (Profitability Index) = VPN_WACC / Equity_MXN * 100  (%)
        # Mide cuántos pesos de VPN se generan por cada 100 pesos de equity aportado.
        # Normaliza por capital propio para comparar eficiencia entre plazos con distinta estructura.
